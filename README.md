...
4. The "CORS" Connection Error:
Challenge: When I first put my app online, the frontend (Netlify) couldn't talk to the backend (Render). The browser blocked it for security reasons, calling it a "CORS error."

Solution: I installed a package called cors in my backend. This tells the server, "It’s okay to accept requests from my Netlify website." It was a great lesson in how different websites talk to each other securely.

5. Dealing with "Slow Starts" on Render:
Challenge: Since I’m using the free version of Render, the server "goes to sleep" if no one uses it. This made the app look broken or empty for the first 30 seconds.

Solution: I added a "Loading..." message in my React code. This tells the user that the app is just waking up, so they don't get confused and leave.
